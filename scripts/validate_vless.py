...
def test_link(xray, link, index, timeout):
    ...
    try:
        ...
        if proc.poll() is not None:
            return index, False, False, False, link, tag, host, port
        ...
        orc, ocode = curl_probe(proxy, "https://chatgpt.com/", timeout)
        chatgpt_ok = orc == 0 and ocode.isdigit() and 200 <= int(ocode) < 400

        return index, True, gemini_ok, chatgpt_ok, link, tag, host, port
    except Exception:
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as ex:
        ...
        for fut in as_completed(futs):
            ...
            i, internet_ok, gemini_ok, chatgpt_ok, _, tag, host, port = r
            print(f"[{i:03d}/{len(links):03d}] net={internet_ok} gemini={gemini_ok} chatgpt={chatgpt_ok} {tag[:35]} {host}:{port}", flush=True)

    good = [r for r in results if r[1] and r[2] and r[3]]
    good = unique_by_endpoint(good)
    good.sort(key=lambda r: (priority(r[5]), -speed_value(r[5]), r[0]))
    lines = [r[4] for r in good]
    Path(args.working).write_text("".join(x + "\n" for x in lines))

    import base64
    encoded = base64.b64encode(("".join(x + "\n" for x in lines)).encode()).decode()
    Path(args.working_base64).write_text(encoded)

    rows = ["status\tgemini\tchatgpt\ttag\thost\tport"]
    for _, internet_ok, gemini_ok, chatgpt_ok, _, tag, host, port in sorted(results, key=lambda r:r[0]):
        rows.append(f"{'WORKS' if internet_ok else 'FAIL'}\t{'YES' if gemini_ok else 'NO'}\t{'YES' if chatgpt_ok else 'NO'}\t{tag}\t{host}\t{port}")
    Path(args.report).write_text("\n".join(rows) + "\n")
    print(f"Published {len(lines)} servers passing internet + Gemini + ChatGPT")
